...
INLINE Score evaluate_king(const Pos *pos, EvalInfo *ei, int Us)
{
  ...
  if (ei->kingAttackersCount[Them] > (1 - popcount(pieces_cp(Them, QUEEN)))) 
  {
    ...
    kingDanger =        ei->kingAttackersCount[Them] * ei->kingAttackersWeight[Them]
                ...
                - 848 * !popcount(pieces_cp(Them, QUEEN))
                -   9 * mg_value(score) / 8
				+  40;
...
INLINE Value evaluate_initiative(const Pos *pos, int asymmetry, Value eg)
{
  ...
  int initiative = 8 * (asymmetry + kingDistance - 17) + 12 * popcount(pieces_p(PAWN)) + 16 * bothFlanks;
